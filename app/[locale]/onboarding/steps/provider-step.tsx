...
import { z } from "zod";
import {
  Loader2, AlertTriangle, ExternalLink, AlertCircle,
  Eye, EyeOff, CheckCircle2, ShieldCheck, Lock,
} from "lucide-react";
...
const PROVIDER_STYLES: Record<ProviderId, { bg: string; border: string; text: string; dot: string }> = {
  openai:    { bg: "bg-emerald-50 dark:bg-emerald-950/20",   border: "border-emerald-200 dark:border-emerald-800",   text: "text-emerald-700 dark:text-emerald-400",   dot: "bg-emerald-500" },
  anthropic: { bg: "bg-orange-50 dark:bg-orange-950/20",     border: "border-orange-200 dark:border-orange-800",     text: "text-orange-700 dark:text-orange-400",     dot: "bg-orange-500" },
  mistral:   { bg: "bg-blue-50 dark:bg-blue-950/20",         border: "border-blue-200 dark:border-blue-800",         text: "text-blue-700 dark:text-blue-400",         dot: "bg-blue-500" },
  gemini:    { bg: "bg-sky-50 dark:bg-sky-950/20",           border: "border-sky-200 dark:border-sky-800",           text: "text-sky-700 dark:text-sky-400",           dot: "bg-sky-500" },
};

export function ProviderStep({ onComplete, onSkip, onBack }: ProviderStepProps) {
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-5">
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <RequiredFormLabel required>{t("providerLabel")}</RequiredFormLabel>
                ...
                {PROVIDER_LIST.map((p) => {
                  ...
                  return (
                    <button
                      key={p.id}
                      type="button"
                      onClick={() => { field.onChange(p.id); setSelectedProvider(p.id); }}
                      className={cn(
                    ...
                    >
                      ...
                      {selected && (
                        <CheckCircle2 className={cn("h-4 w-4 shrink-0", s.text)} />
                      )}
                    </button>
                  );
                })}
        ...
        <div className="flex items-center gap-2 rounded-lg border border-border bg-muted/30 px-3 py-2.5">
          ...
          <Badge variant="secondary" className="ml-auto text-[10px] shrink-0">AES-256</Badge>
        </div>
        ...
        <Button type="submit" variant="cta-primary" size="lg" className="w-full" disabled={isSubmitting}>
          {isSubmitting ? (
            ...
            <><ShieldCheck className="mr-2 h-4 w-4" />{t("submit")}</>
          )}
        </Button>
