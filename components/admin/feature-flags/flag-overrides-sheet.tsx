...
import { addOverride, removeOverride } from "@/app/[locale]/(admin)/admin/feature-flags/_actions";
...
export function FlagOverridesSheet({
  ...
  const [enabledValue, setEnabledValue] = React.useState<"true" | "false">("true");
  ...
  async function handleAdd(e: React.FormEvent) {
    ...
    setAdding(true);
    setAddError(null);
    const result = await addOverride(flagId, email.trim(), enabledValue === "true");
    setAdding(false);
    if (!result.success) {
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="right" className="w-full sm:max-w-md flex flex-col gap-0 p-0">
        <SheetHeader className="p-4 border-b">
          ...
          <SheetDescription className="font-mono text-xs">{flagKey}</SheetDescription>
        </SheetHeader>
        ...
        <form onSubmit={handleAdd} className="p-4 border-b flex flex-col gap-3">
          ...
          <div className="flex gap-2">
            ...
            />
            <Select value={enabledValue} onValueChange={(v) => setEnabledValue(v as "true" | "false")}>
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {overrides.length === 0 ? (
            <p className="text-sm text-muted-foreground text-center py-8">{t("overrides.empty")}</p>
          ) : (
              ...
              {overrides.map((override) => (
                ...
                >
                  <div className="flex flex-col min-w-0">
                    <span className="text-sm truncate">{override.user.email}</span>
                    {override.user.name && (
                      <span className="text-xs text-muted-foreground truncate">{override.user.name}</span>
                    )}
                  </div>
                  <div className="flex items-center gap-2 shrink-0">
                    <Badge variant={override.enabled ? "default" : "secondary"} className="text-xs">
                      {override.enabled ? t("overrides.enabled") : t("overrides.disabled")}
                    </Badge>
                    <Button
                      variant="ghost"
                      size="sm"
