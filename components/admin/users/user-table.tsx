            ...
            return (
              ...
              >
                    ...
                    {user.roles.length === 0 ? (
                      ...
                      user.roles.map((r) => (
                        <Badge
                          key={r.name}
                          variant="outline"
                          className={`text-[10px] font-semibold uppercase tracking-wider h-5 px-1.5 ${ROLE_STYLE[r.name] ?? ROLE_STYLE.VIEWER}`}
                        >
                          {r.displayName}
                        </Badge>
                      ))
                ...
                <TableCell onClick={(e) => e.stopPropagation()}>
                  <Badge
                    variant="outline"
                    className={`text-[10px] font-semibold uppercase tracking-wider h-5 px-1.5 ${PLAN_STYLE[user.plan]}`}
                  >
                    {user.plan}
                  </Badge>
                </TableCell>
                <TableCell onClick={(e) => e.stopPropagation()}>
                  {isFeedback ? (
                    <span
                      className={`text-xs font-medium ${feedback.ok ? "text-secondary-600" : "text-error-600"}`}
                    >
                      {feedback.msg}
                    </span>
                  ) : (
