...
export function UserStats({ users, t }: Props) {
  ...
  const stats = [
    { label: t("stats.total"),     value: users.length, icon: Users,     color: "text-muted-foreground" },
    { label: t("stats.active"),    value: active,        icon: UserCheck, color: "text-secondary-600 dark:text-secondary-400" },
    { label: t("stats.suspended"), value: suspended,     icon: UserX,     color: "text-amber-600 dark:text-amber-400" },
    { label: t("stats.banned"),    value: banned,        icon: Ban,       color: "text-error-600 dark:text-error-400" },
  ];
