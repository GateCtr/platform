...
describe("Property 16: Upsell dedup via Redis", () => {
  ...
  it(
    "returns show:false when Redis dedup key already exists (>=100 iterations)",
    async () => {
      const { getUpsellState } = await import(
        "@/app/[locale]/(dashboard)/billing/_actions"
      );
  ...
  it(
    "returns show:true on first hit when Redis key is absent (>=100 iterations)",
    async () => {
      const { getUpsellState } = await import(
        "@/app/[locale]/(dashboard)/billing/_actions"
      );
  ...
  it(
    "Redis SET is called with nx:true and ex:86400 for dedup key (>=100 iterations)",
    async () => {
      const { getUpsellState } = await import(
        "@/app/[locale]/(dashboard)/billing/_actions"
      );
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (clerkId, userId) => {
            ...
            mockUserFindUnique.mockResolvedValue({ id: userId, plan: "FREE", clerkId });
            mockGetPlanLimits.mockImplementation(async (plan: string) => {
              if (plan === "FREE") return makeFreeLimits();
              if (plan === "PRO") return makeProLimits();
              return null;
