...
describe("Property 19: GateCtr webhook fired for every plan change", () => {
  ...
  it("dispatchWebhook called with billing.plan_upgraded after checkout.session.completed (>=100 iterations)", async () => {
  ...
  it("dispatchWebhook called with billing.plan_downgraded after customer.subscription.deleted (>=100 iterations)", async () => {
    const { POST } = await import("@/app/api/webhooks/stripe/route");
  ...
  it("dispatchWebhook called with billing.payment_failed after invoice.payment_failed (>=100 iterations)", async () => {
    const { POST } = await import("@/app/api/webhooks/stripe/route");
  ...
  it("dispatchWebhook called with billing.trial_started after subscription enters trialing (>=100 iterations)", async () => {
    const { POST } = await import("@/app/api/webhooks/stripe/route");
